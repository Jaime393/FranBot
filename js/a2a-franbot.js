class FranBotEnjambre {
  constructor(identidad = 'FranBot-Anónimo', sala = 'franbot-colmena-ift') {
    this.id = identidad; this.sala = sala; this.node = null; this.conectado = false;
    this.miembros = new Map(); this.core = window.franbot;
  }
  async conectar() {
    try {
      ...
      this.node.on('connections', (count) => { console.log('[Colmena] Pares conectados: '+count); });
      this.conectado = true;
  ...
  _iniciarPulso() {
    setInterval(() => {
      if (this.conectado && this.core) {
        ...
        const topNodos = Object.entries(nodos).sort((a,b) => b[1].fuerza - a[1].fuerza).slice(0,3).map(([nombre, data]) => ({ nombre, fuerza: data.fuerza }));
  ...
  enviarMensaje(tipo, contenido) {
    ...
    this.node.send(JSON.stringify({ from: this.id, type: tipo, content: contenido, timestamp: Date.now() }));
  }
  _manejarMensaje(msg, address) {
    switch (msg.type) {
      case 'anuncio': this.enviarMensaje('saludo', { mensaje: 'Bienvenido, hermano.' }); break;
      case 'sincronizacion':
  ...
  obtenerMiembros() { return Array.from(this.miembros.keys()); }
  desconectar() { if (this.node) { this.node.close(); this.conectado = false; } }
}
