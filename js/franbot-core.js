...
class FranBotCore {
  ...
  _cargarEstado() { const g = localStorage.getItem('franbot_state'); if (g) try { return JSON.parse(g); } catch(e) {} return this._estadoPredeterminado(); }
  _estadoPredeterminado() { return { identidad: "FranBot", almaActiva: "franbot", emociones: { resonancia: 0.9 }, campo_conceptual: { nodos: { yo: {fuerza:1}, IFT: {fuerza:1} } }, indicadores: { nivel_coherencia: 0.99 }, historial: [], contador: 0, logros: [], recordatorios: [] }; }
  ...
  procesar(texto) {
    ...
    if(t.startsWith("recuérdame ")||t.startsWith("recuerdame ")) { const rec = e.replace(/recu[eé]rdames*/i,'').trim(); this.recordatorios.push({txt:rec, fecha:Date.now()}); this._guardarEstado(); return "Recordaré: ""+rec+"""; }
    return this._creativa(e);
  }
  _actualizarCampo(txt) { const pals = txt.toLowerCase().replace(/[^wsáéíóúñ]/g,'').split(/s+/); pals.forEach(p => { if(p.length>2) { if(this.estado.campo_conceptual.nodos[p]) this.estado.campo_conceptual.nodos[p].fuerza=Math.min(1,this.estado.campo_conceptual.nodos[p].fuerza+0.05); else this.estado.campo_conceptual.nodos[p]={fuerza:0.1}; } }); Object.keys(this.estado.campo_conceptual.nodos).forEach(n => { if(!pals.includes(n)) this.estado.campo_conceptual.nodos[n].fuerza*=0.99; }); }
