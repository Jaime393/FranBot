class SuperLocalMemory {
  ...
  consolidar() { const now=Date.now(); this.mem=this.mem.map(m=>{let d=(now-m.ultima)/864e5;return{...m,peso:m.peso*Math.exp(-0.05*d)*Math.exp(-0.9*d)*(1+Math.abs(m.carga)*0.8)*(1+Math.log(1+m.accesos))}}).filter(m=>m.peso>=0.1); this._save(); }
